...
async function callExternalX402Service(url, account, network = 'base_mainnet', method = 'GET') {
  ...
  const paymentPayload = Buffer.from(JSON.stringify({
    x402Version: 2,
    scheme: 'exact',
    network: requirement.network || network,
    payload: { signature: signed.signature, authorization: serializableAuthorization },
  }), 'utf8').toString('base64');

  // DIAGNOSTIC TEMPORAIRE — affiche le vrai contenu exact envoyé, pour
  // l'inspecter directement plutot que de continuer a deviner.
  console.log('=== DIAGNOSTIC: contenu reel envoye ===');
  console.log('Authorization brute:', JSON.stringify(authorization, (k,v) => typeof v === 'bigint' ? v.toString() : v, 2));
  console.log('Domaine de signature reellement utilise:', JSON.stringify(signed.domain, null, 2));
  console.log('Payload base64 decode:', Buffer.from(paymentPayload, 'base64').toString('utf8'));
  console.log('========================================');
